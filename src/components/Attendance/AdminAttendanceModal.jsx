        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/60">
            <div className="flex items-center gap-2 text-slate-100 font-bold text-base">
              <ShieldAlert className="h-5 w-5 text-nexora-purple" />
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-sm focus:outline-none focus:border-nexora-purple"
              >
                {users.map((u) => (
            ...
            {/* Audit History Timeline (if editing existing) */}
            {initialRecord?.editHistory && initialRecord.editHistory.length > 0 && (
              <div className="pt-2 border-t border-slate-800/80">
                <span className="text-xs font-semibold text-slate-400 flex items-center gap-1.5 mb-2">
                  <History className="h-3.5 w-3.5 text-nexora-purple" />
                  Edit Audit Trail
                </span>
                <div className="max-h-24 overflow-y-auto space-y-1.5 text-[11px] bg-slate-950 p-2.5 rounded-xl border border-slate-800">
                  {initialRecord.editHistory.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between text-slate-400 border-b border-slate-800/50 pb-1 last:border-0">
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
              ...
              <button
                type="submit"
                disabled={saving}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-nexora-indigo to-nexora-purple text-white text-xs font-bold shadow-glow-purple cursor-pointer disabled:opacity-50"
              >
