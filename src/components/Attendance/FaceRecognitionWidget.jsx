...
export default function FaceRecognitionWidget({ currentUser, onScanComplete }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 md:p-8 rounded-2xl bg-white border border-slate-200 shadow-sm text-center space-y-6">
      ...
      <div className="relative w-64 h-64 md:w-72 md:h-72 rounded-3xl bg-slate-900 border-2 border-slate-300 flex items-center justify-center overflow-hidden shadow-lg">
        <img 
        ...
        {success && (
          <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-xs flex flex-col items-center justify-center gap-2 text-emerald-400 p-4">
            ...
            <span className="text-sm font-bold text-white">Face Match 99.8% Confirmed</span>
            <span className="text-xs text-slate-300">Biometric Check-In Recorded</span>
          </div>
        )}
      </div>
      ...
      <motion.button
        ...
        disabled={scanning}
        className="py-3 px-6 rounded-xl bg-cyan-600 hover:bg-cyan-700 text-white font-bold text-xs shadow flex items-center gap-2 cursor-pointer disabled:opacity-50"
      >
